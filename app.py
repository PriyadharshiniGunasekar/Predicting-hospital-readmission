...
try:
    ...
    st.error("❌ Model or scaler not found. Ensure both 'xgb_model.pkl' and 'scaler.pkl' are inside 'models/' folder.")
    st.stop()
...
if submit:
    ...
    gender_map = {"Male": 1, "Female": 0, "Other": 2}  # note: train.py mapped Male=1, Female=0
    ...
    # ---- Feature Engineering (exactly like train.py) ----
    ...
    if prediction == 1:
        st.error(f"🔴 High Risk: Patient likely to be readmitted (probability {prediction_proba:.2f})")
    else:
        st.success(f"🟢 Low Risk: Patient unlikely to be readmitted (probability {prediction_proba:.2f})")
